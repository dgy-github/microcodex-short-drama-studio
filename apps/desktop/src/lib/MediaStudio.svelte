<script lang="ts">
  ...
  let activeRun = $state("");
  let message = $state("");
  let toolStatus = $state<Array<{ id: string; version: string; status: string }>>([]);
  ...
  onMount(async () => {
    ...
    toolStatus = (await desktopApi.diagnoseMediaTools().catch(() => ({ tools: [] }))).tools;
  });
</script>
...
<section class="studio">
  ...
  <article class="gateway">
    <h3>可信媒体网关</h3>
    <div class="tool-status" aria-label="媒体工具链状态">
      <strong>本地工具链</strong>
      {#if toolStatus.length === 0}<span>未检测到可信 manifest</span>{/if}
      {#each toolStatus as tool}<span class:ready={tool.status === "ready"}>{tool.id} {tool.version} · {tool.status}</span>{/each}
    </div>
