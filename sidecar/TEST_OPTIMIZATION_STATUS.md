...
> 历史快照，不能作为当前测试清单或待办事实源。请以当前测试源码和门禁命令的实际
> 输出为准；下面的 skip/TODO 状态可能已经变化。
...
> 历史快照说明（2026-08-30）：本文记录的 Vitest skip/TODO 清单不是当前事实源。
> 当前桌面测试已通过 141/141；保留本文仅为历史审计，新增状态以 `npm test -- --run`
> 和对应测试源码为准。
